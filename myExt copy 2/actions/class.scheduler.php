...
class myExt_actions_Scheduler extends tao_actions_CommonModule {
	...
	public function run(){
       ...
       //print_r($statement);
        ...
        while ($row = $result->fetch()){
        	...
        	$scheduledDeliveries[$row['deliveryID']]['deliveryID'] = $row['deliveryID'];
        	$scheduledDeliveries[$row['deliveryID']]['endDate'] = $row['endDate'];
        }
		
		foreach($scheduledDeliveries as $delivery){
			$this->sendEmails($delivery['deliveryID'], $delivery['endDate']);
	...
	//To DO: See if there is a away to get the Delivery OBJECT not only URI. 
	 protected function sendEmails($uri, $endDate){
        ...
        foreach($recipients as $recipient)
		{
		   ...
		   $message->setBody("<p>Dear ".$recipient['firstName'].' '.$recipient['lastName'].",</p> <p>a new test is available for you. Please login to TAO tool to take the test. The deadline to take this test is ".date('l, d F Y ',strtotime($endDate))."</p>");
