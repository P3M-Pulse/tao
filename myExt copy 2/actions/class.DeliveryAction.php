...
class myExt_actions_DeliveryAction extends taoDelivery_actions_Delivery
{
 ...
 public function __construct()
    ...
    public function scheduleEmails(){
    	...
    	$query = 'SELECT startDate, endDate FROM delivery_reminders WHERE deliveryID = ? ';
        $result = $dbWrapper->query($query, array($delivery->getUri()));
       	$queryResult = $result->fetch();
        if(empty($queryResult)){
        	$query = 'INSERT INTO delivery_reminders (startDate, endDate, updated, deliveryID)
        			VALUES  (?, ?, ?, ?)';
        	$runQuery = true;		
        }else{
        	...
        	if($startDate != strtotime($currentStartDate) || $endDate != strtotime($currentEndDate)){ 
        		$query = ' UPDATE delivery_reminders set startDate = ?, endDate = ?, updated = ? where deliveryID = ?';
        		$runQuery = true;
        	}
        }
    	...
    	if(!empty($startDate) && !empty($endDate) && $runQuery==true){
   		...
   		/*
$dbWrapper = core_kernel_classes_DbWrapper::singleton();
    	$query = 'SELECT * FROM statements WHERE object =\'Danish\' ';
        $result = $dbWrapper->query($query);
        $statement = $result->fetch();
        print_r($statement);
*/
		/*
$query = 'INSERT INTO delivery_reminders (deliveryID, startDate, endDate, updated)
        			VALUES  (?, ?, ?, ?)';
